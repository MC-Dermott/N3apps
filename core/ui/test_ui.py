...
def _is_correct(user_input, expected):
    try:
        student = float(str(user_input).replace(",", "").replace("£", "").strip())
...
def _render_summary(test):
    ...
    if score == _NUM_QUESTIONS:
        ...
        if remaining > 0:
            timer_block = f"""
                <div id="n3-timer-text" style="font-family:sans-serif;text-align:center;color:#eaeaea;margin:6px 0 0;">
                    ⏱ Game available for <span id="n3-secs">{remaining}</span> seconds
                </div>
                <script>
                (function() {{
                    var s = {remaining};
                    var secsEl = document.getElementById('n3-secs');
                    var timerText = document.getElementById('n3-timer-text');
